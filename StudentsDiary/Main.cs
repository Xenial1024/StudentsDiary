...
namespace StudentsDiary
{
    public partial class Main : Form
    {
        ...
        FileHelper<List<Student>> _fileHelper = new FileHelper<List<Student>>(Program.FilePath);
        ...
        public Main()
        {
            ...
            RefreshDiary();
            SetColumnsHeader();
            //ChangeDgvDiaryVisibleRows();

            if (IsMaximize)
                WindowState = FormWindowState.Maximized;

            cobViewGroup.SelectedItem = "wszystkich klas";
            // Wybranie wyświetlenia uczniów z wszystkich klas jako domyślnej opcji
            cobViewGroup.SelectedIndexChanged += CobViewGroup_SelectedIndexChanged; 
        }
        ...
        private void ChangeDgvDiaryVisibleRows()
        {
            
            if (cobViewGroup.SelectedIndex > 0)// Jeśli wybrano coś innego niż "wszystkich klas"
                dgvDiary.DataSource = students.Where(x => x.Group == cobViewGroup.SelectedItem.ToString()).ToList();
            else // Jeśli wybrano "wszystkich klas"
                dgvDiary.DataSource = students; 
        }

        private void CobViewGroup_SelectedIndexChanged(object sender, EventArgs e)
        {
            ChangeDgvDiaryVisibleRows(); 
        }
